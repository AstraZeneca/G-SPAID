...
calculate_AUC <- function(index, d) {
  ...
  if (isTRUE(all(gd == gd_hi|gd == gd_lo ))) {
    ...
    runs <- rle(g30$logiclo > 0)         ### get Consecutive runs 
    ...
    runs <- rle(g30$logicho > 0)         ### get Consecutive runs
    Trun <- which(runs$values == TRUE) ### Consecutive TRUE
    runs.lengths.cumsum = cumsum(runs$lengths)
    ends = runs.lengths.cumsum[Trun]
    newindex = ifelse(Trun>1, Trun-1, 0)
...
```{r}
dt <- import("") %>% as.data.frame


cell_type <- c("B cell","Treg","Neutrophils","F4/80+ CD68+")
distance <- 30
sim <- 30 
m <- 5
count_cell <- 10

#colnames(dt)[colnames(dt) == 'Image Location'] <- 'Image'
colnames(dt)[colnames(dt) == 'group'] <- 'Group'
dt$X <- (dt$XMin + dt$XMax)/2
dt$Y <- (dt$YMin + dt$YMax)/2
dt <- dt[,c("Image",cell_type,"X","Y","Group")]
...
normfname <- file.path(outDir,("gcluster.csv"))
write.csv(AUC_df, normfname)
...
normfname <- file.path(outDir,("gdispersion_dist.csv"))
...
normfname <- file.path(outDir,("gcluster_dist.csv"))
write.csv(dist_df, normfname,row.names = FALSE)
...
#write.csv(df_n,outDir,row.names = FALSE)
...
print(Heatmap(mean_matrix, column_split = factor(order_col,levels = unique(order_col)),na_col = "grey", col = f1, column_names_gp = gpar(fontsize = 8),column_title_rot = 45,column_title_gp = gpar(fontsize = 10,fontface="bold"),row_names_gp = gpar(fontsize = 10,fontface="bold"),show_column_dend = FALSE, show_row_dend = FALSE,cluster_columns = FALSE,cluster_rows = FALSE,cluster_column_slices = FALSE,name = "AUC",row_title = "i", column_names_rot = 90,
...
for (int in sig_interaction) {
  ...
  for (diet in unique(dt$Group)) {
    ...
    for (x in unique(sub$Image)) {
      ...
      if ((length(p1x) <= 1 | length(p2x) <= 1) == TRUE) {
        ...
        pm = ppp(c(p1x, p2x),
                 c(p1y, p2y),
                 window = window,
                 marks = factor(c(rep(i, p1$n), rep(j, p2$n))))
        g <-
          envelope(pm,Gcross,i = i,j = j,nsim = sim, fix.n = TRUE,fix.marks = TRUE,savefuns = TRUE,nrank = m,verbose = FALSE) # Gcross
        plot(g,legend = FALSE,xlab = "",ylab = "",cex.axis = 0.8,cex.main = 0.8,main = x)
    ...
    ### plot histogram
    nd2 <-
      as.data.frame(list(nnd_density, subject), col.names = c("nn", "sample_ID"))
    plot.new()
    pdf(
      paste0(diet, "_H-%03d.pdf"),
      width = 11.4,
      height = 6.5,
      onefile = FALSE
    )
  ...
  df_summary <-
    as.data.frame(
      ...
      col.names = c("ID", i, j, "AUC", "dist_list", "average_NND", "group")
    )
  ...
  ggplot(df_summary, aes(x = group, y = AUC)) + geom_boxplot(aes(fill = group),
                                                             outlier.shape = NA,
                                                             width = 0.4) +
    geom_point(position = position_dodge(width = 0.75), aes(group = group)) + theme_bw() + theme_classic() +
    stat_pvalue_manual(
      bracket.nudge.y = max(df_summary$AUC),
      stat.test2,
      label = "p.signif",
      tip.length = 0,
      hide.ns = TRUE,
...
p1x <- win$X[win[i] > 0]
p1y <- win$Y[win[i] > 0]
p1_list <- cbind(x=p1x,y=p1y)
nn = get.knnx(p1_list,p1_list ,k = 2)
filename <- paste(outDir,i,x,".pdf",sep="")
plot.new()
pdf(filename,width=8.5, height=8.5)
plot(cbind(x=win$X,y=win$Y),pch = 16, col = alpha("grey",0.3),cex =0.6,xlab="",ylab="")
points(p1_list, pch = 16, col = 1,cex =0.6)
for(k in 1:nrow(p1_list)) lines(lty = 1,x = c(p1_list[k,1], p1_list[nn$nn.index[k,2],1]),
                                y = c(p1_list[k,2], p1_list[nn$nn.index[k,2],2]))
legend(x = "topright",legend = i,col=1,pch=16)
dev.off()
...
B <- data.frame(m)
colnames(B) <- seq(length(p1x))
B$which_i <- rownames(B)
pairdist1 <- B %>% pivot_longer(cols =  where(is.numeric),values_to = "distance",
                                names_to = "which_j") %>% filter(distance < dr)
pairdist1$which_i <- as.numeric(pairdist1$which_i)
pairdist1$which_j <- as.numeric(pairdist1$which_j)
filename <- paste(outDir,dr,i,x,".pdf",sep="")
plot.new()
pdf(filename,width=8.5, height=8.5)
plot(cbind(x=win$X,y=win$Y),pch = 16, col = alpha("grey",0.3),cex =0.6,xlab="",ylab="")
points(p1_list, pch = 16, col = 1,cex =0.6)
for(k in 1:nrow(pairdist1)) lines(lty = 1,x = c(p1_list[pairdist1[[k,1]],1], p1_list[pairdist1[[k,2]],1]),
                                  y = c(p1_list[pairdist1[[k,1]],2], p1_list[pairdist1[[k,2]],2]))
legend(x = "topright",legend = c(i),col=c(1,2),pch=16)
dev.off()
```
...
p2x <- win$X[win[j] > 0]
p2y <- win$Y[win[j] > 0]
nmds_df <- as.data.frame(list(p1x,p1y),col.names= c("x","y"))
p1_list <- cbind(x=p1x,y=p1y)
p2_list <- cbind(x=p2x,y=p2y)
nn = get.knnx(p2_list, p1_list, k = 1)
filename <- paste(outDir,i,j,x,".pdf",sep="")
plot.new()
pdf(filename,width=8.5, height=8.5)
plot(cbind(x=win$X,y=win$Y),pch = 16, col = alpha("grey",0.3),cex =0.6,xlab="",ylab="")
points(p1_list, pch = 16, col = 1,cex =0.6)
points(p2_list, pch = 16, col = 2,cex =0.6)
for(k in 1:nrow(p1_list)) lines(lty = 1,x = c(p1_list[k,1], p2_list[nn$nn.index[k,1],1]),
                                y = c(p1_list[k,2], p2_list[nn$nn.index[k,1],2]))
legend(x = "topright",legend = c(i, j),col=c(1,2),pch=16)
dev.off()

### specific range plot
dr <- 30
m <- crossdist(p1x,p1y,p2x,p2y)
B <- data.frame(m)
colnames(B) <- seq(length(p2x))
B$whichi <- rownames(B)
pairdist1 <- B %>% pivot_longer(cols =where(is.numeric),values_to = "distance",
                                names_to = "whichj") %>% filter(distance < dr)
pairdist1$whichi <- as.numeric(pairdist1$whichi)
pairdist1$whichj <- as.numeric(pairdist1$whichj)
filename <- paste(outDir,dr,i,j,x,".pdf",sep="")
plot.new()
pdf(filename,width=8.5, height=8.5)
plot(cbind(x=win$X,y=win$Y),pch = 16, col = alpha("grey",0.3),cex =0.6,xlab="",ylab="")
points(p1_list, pch = 16, col = 1,cex =0.6)
points(p2_list, pch = 16, col = 2,cex =0.6)
for(k in 1:nrow(pairdist1)) lines(lty = 1,x = c(p1_list[pairdist1[[k,1]],1], p2_list[pairdist1[[k,2]],1]),
                                  y = c(p1_list[pairdist1[[k,1]],2], p2_list[pairdist1[[k,2]],2]))
legend(x = "topright",legend = c(i, j),col=c(1,2),pch=16)
dev.off()
```
